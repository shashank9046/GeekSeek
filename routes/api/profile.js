...
router.get('/user/:user_id', async (req,res)=>{
    try {
        const profile =await Profile.findOne({user: req.params.user_id}).populate('user',['name','avatar'],User); //path,select,model
        if(!profile)
...
,async (req,res)=>{
    ...
    const newExp={
     ...
     try {
        ...
        profile.experience.unshift(newExp);//unshifts insert from begining
        await profile.save();
        res.json({profile});
    } catch (err) {
        console.error(err.message);
...
,async (req,res)=>{
    ...
    const{
        college,branch,course,from,to,current,description
    }=req.body;
    const newEdu={
        college,branch,course,from,to,current,description
    }
     try {
        const profile = await Profile.findOne({user: req.user.id}); 
        profile.education.unshift(newEdu);//unshifts insert from begining
        await profile.save();
        res.json({profile});
    } catch (err) {
        console.error(err.message);
